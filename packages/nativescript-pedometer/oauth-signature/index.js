...
(function () {
  ...
  OAuthSignature.prototype.generate = function (
    httpMethod,
    url,
    parameters,
    consumerSecret,
  ...
  UrlElement.prototype = {
    get: function () {
      ...
      var parsedUrl = this.parse(), // FIXME: Make this behaviour explicit by returning warnings
        scheme = (parsedUrl.scheme || 'http').toLowerCase(), // FIXME: Make this behaviour explicit by returning warnings
    ...
    parse: function () {
      return {
        scheme: UrlParser('protocol', this._url).toLowerCase(),
        authority: UrlParser('hostname', this._url).toLocaleLowerCase(),
        port: UrlParser('port', this._url),
        path: UrlParser('path', this._url),
      };
    },
  };
  ...
  function HmacSha1Signature(signatureBaseString, consumerSecret, tokenSecret) {
    ...
    this._key =
      this._rfc3986.encode(consumerSecret) +
      '&' +
      this._rfc3986.encode(tokenSecret);
    this._base64EncodedHash = new HmacSha1(
      this._text,
      this._key
